...
class ImportSelectedData(models.TransientModel):
    ...
    fetched_ids = fields.Many2many('hydrofarm.outputs', 'hydrofarm_group_selected_rel', string="OutPut", default=lambda self: self._get_outputs(), required=True)
    ...
    def import_selected_data(self):
        ...
        try:
            for prd in self.fetched_ids:
                vendor = prd.connection_id.partner
                if vendor:
                            ...
                            for line1 in vendor_mapping1:
                                ...
                                if line1.vendor_api_ref == 'image':
                                    ...
                                    if img != False:
                                        ...
                                        # response = requests.get(prd.image)
                                        # if response.status_code == 200:
                                        #     image_bytes = BytesIO(response.content)
                                        #     image_base64 = codecs.encode(response.content, 'base64')
                                        #
                                        #     fp = tempfile.NamedTemporaryFile( suffix= "." + str(prd.image).split(".",-1)[-1] )
                                        #     fp.write(binascii.a2b_base64(image_base64))
                                        #
                                        #
                                        # pdb.set_trace()
